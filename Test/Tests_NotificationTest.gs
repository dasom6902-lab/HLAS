...
function cleanupNotificationFixtures_(marker, taskIds) {
  ...
  SheetRepository.findAll(C.SHEETS.NOTIFICATION).forEach(function (record) {
    const id = String(record[C.NOTIFICATION_FIELD.NOTIFICATION_ID] || '');
    const entityId = String(record[C.NOTIFICATION_FIELD.ENTITY_ID] || '');
    const title = String(record[C.NOTIFICATION_FIELD.TITLE] || '');
    if (
