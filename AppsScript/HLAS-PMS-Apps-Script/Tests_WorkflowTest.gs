...
function runWorkflowTests() {
  ...
  row[C.FIELD.PROJECT.PROJECT_NAME] = 'TASK-0016 Workflow Test';
  row[C.FIELD.PROJECT.STATUS] = S.DRAFT;
  row[C.FIELD.PROJECT.CREATED_AT] = new Date();
  row[C.FIELD.PROJECT.UPDATED_AT] = new Date();
  ...
  try {
    ...
    const oldTask = {};
    oldTask[C.FIELD.TASK.TASK_ID] = 'TASK-WF-RULE';
    oldTask[C.FIELD.TASK.TASK_NAME] = '승인 대기 테스트';
    oldTask[C.FIELD.TASK.STATUS] = S.WAITING_APPROVAL;
    oldTask[C.FIELD.TASK.OWNER] = 'test';
    oldTask[C.FIELD.TASK.UPDATED_AT] = new Date(Date.now() - 48 * 60 * 60 * 1000);
    assertWorkflowTest_(
